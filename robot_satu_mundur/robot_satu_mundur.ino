...
int muter; //Untuk menghitung berapa kali berputar

//For stop
unsigned long lastDetectionTime = 0;
const unsigned long noDetectionTimeout = 1500; //default 700, custom 1 = 1500
...
void bacaSensor() {
  ...
  else if (digitalRead(sens_tengah) == 0 && digitalRead(sens_kiri1) == 0 && digitalRead(sens_kiri2) == 0 && digitalRead(sens_kanan1) == 0 && digitalRead(sens_kanan2) == 0){
    lastDetectionTime = millis();
    if (millis() - lastDetectionTime < noDetectionTimeout){
      gerakMaju();
    } else {
      if (digitalRead(sens_tengah) == 1 && digitalRead(sens_kiri1) == 1 && digitalRead(sens_kiri2) == 1 && digitalRead(sens_kanan1) == 1 && digitalRead(sens_kanan2) == 1){
        stopMotors();
        // delay(500);
        putarBalik();
        // delay(500);
...
void putarBalik() {
  ...
  while (true) {
    ...
    if (digitalRead(sens_kiri1) == 0 || digitalRead(sens_kiri2) == 0) {
      ...
      stopMotors();
      muter++; //Menghitung kondisi robot sudah berputar
